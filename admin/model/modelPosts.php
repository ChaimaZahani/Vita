...
class ModelPosts
{
    ...
    private $author_id;
    private $is_featured;
    public function __construct($title = null, $body = null, $thumbnail = null, $category_id = null, $author_id = null, $is_featured = null)
    {
        if (!is_null($title) && !is_null($body) && !is_null($thumbnail) && !is_null($category_id) && !is_null($author_id) && !is_null($is_featured)) {
    ...
    {
        $db = App::getDB();
        $query = "SELECT id, title,category_id FROM posts WHERE author_id=$id ORDER BY id DESC";
        $params = array($id);
        $resultat = $db->execute_query($query, $params);
        if (!$resultat) {
            echo "Lecture impossible";
        } else {
            return $resultat;
    ...
    public function save($id = null)
    {
        ...
        try {
            if ($id == null) { //insertion d’un nouveau produit
                $sql = "INSERT INTO posts(title, body,thumbnail,category_id,author_id,is_featured) VALUES (?,?,?,?,?,?)";
                $params = array(
                    $this->title,
                    $this->body,
                    $this->thumbnail,
    ...
    public static function getThumbnail($id)
    {
        $db = App::getDB();
        try {
            $sql =  "SELECT thumbnail FROM posts WHERE author_id=$id";
            $params = array($id);
            $resultat = $db->execute_query($sql, $params);
        } catch (PDOException $e) {
            echo "Lecture impossible, id";
    ...
    public static function update_featured()
    {
        $db = App::getDB();

        $sql = "UPDATE posts SET is_featured=0";
        $resultat = $db->execute_query($sql);
        if (!$resultat) {
            echo "Lecture impossible, id";
        } else {
    ...
    public static function update_user($firstname,$lastname,$is_admin,$id)
    {
        $db = App::getDB();

        $sql = "UPDATE user SET firstname='$firstname', lastname='$lastname', is_admin=$is_admin WHERE id=$id LIMIT 1";
        $resultat = $db->execute_query($sql);
        if (!$resultat) {
            echo "Lecture impossible, id";
        } else {
    ...
    public static function update_posts($title,$body,$thumbnail_to_insert,$category_id,$is_featured,$id)
    {
        $db = App::getDB();
